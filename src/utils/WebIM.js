...
WebIM.conn.listen({
    onOpened: function (message) { // 连接成功回调
        // 登录或注册成功后 跳转到好友页面
        console.log("%c 链接成功", "color:green")
    ...
    onTextMessage: function (message) {
        ...
        let messageTo = message.ext.chatType === "friend" ? message.from : message.to;

        layui.layim.getMessage({
            username: message.from //消息来源用户名
            , avatar: "/statics/layui/images/head.png" //消息来源用户头像
            , id: messageTo //消息的来源ID（如果是私聊，则是用户id，如果是群聊，则是群组id）
            , type: message.ext.chatType //聊天窗口来源类型，从发送消息传递的to里面获取
            , content: message.sourceMsg //消息内容
            , cid: message.id //消息id，可不传。除非你要对消息进行一些操作（如撤回）
            , mine: false //是否我发送的消息，如果为true，则会显示在右方
            , fromid: message.from //消息的发送者id（比如群组中的某个消息发送者），可用于自动解决浏览器多窗口时的一些问题
            , timestamp: Number(message.ext.timestamp) //服务端时间戳毫秒数。注意：如果你返回的是标准的 unix 时间戳，记得要 *1000
        })
